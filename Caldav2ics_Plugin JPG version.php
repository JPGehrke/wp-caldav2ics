...
Jan 2023: several modifications in the code made by Jörg-Peter Gehrke, sent to W. Joss for review
25.02.2023: W. Joss published an update Version 1.3.2 of the plugin (see https://hoernerfranzracing.de/werner/blog/2023-wp-caldav2ics-reloaded) 
25-Feb-2023 This is the private version of the plugin file - enters the exclude phrase in the wp-options table 'caldav2ics_calendar_excludes' - JPG
*/
...
class Caldav2ics_Plugin extends Caldav2ics_LifeCycle {
	...
	public function getOptionMetaData() {
		return array(
			...
			'xfilter' => array(__('Enable defintion of exclude phrase !','wp-caldav2ics'),'true','false'),	// JPG 02FEB2023		
		);
	}
	...
	protected function initOptions() {			// WJ modification Feb 2023
...
// The ics files from my CALDAV server may contain property info in long wrapped lines. By just "trimming" also leading spaces of the wrapped lines will be eliminated. 
...
//			print_r($CalendarOptions);				// should this line be deleted or is it needed for debugging ?
		$GeneralOptionMetaData = $this->getOptionMetaData();
			if (($this->getOption("xfilter","true") == 'true') )	{		//JPG 03FEB2022
		 		$exclude = true;
				}	
			else		{
				$exclude = false;
			...
			if (isset($_POST['updateSettings']) || isset($_POST['updateSettingsjpg'])) {	// 'submit' Button pressed ? -> show Message
...
//			print "jpg exclude : ".$exclude. "after if isset function ! ";
			$cal_url_Array=array();
			...
			$cal_exclude_Array=array();
			foreach($_POST['caldav2ics_calendar_excludes'] as $k=>$val) {		// JPG 12FEB23 filling $cal_exclude_Array
				if(!empty(trim($val)) && $exclude) {				// writes $val in $cal_exclude_Array if $exclude = TRUE and $val NOT empty
					$cal_exclude_Array[$k+1] = trim($val);
				}
              			  if (empty(trim($val)) && ($k+1 <= count($cal_files_Array)) && $exclude) { // writes "lore ipsum" if NO "exclude phrase" is defined for calendar url & $exclude=TRUE
              			  							// ensures that $cal_exclude_Array contains "exclude phrase" at correct position in serial database
			       	$cal_exclude_Array[$k+1] = "lore ipsum";    		
				}
				    if (!$exclude && ($k+1 <= count($cal_files_Array))) {	// fills $cal_exclude_Array if $exclude is disabled with "lore ipsum" 
												// writes "lore ipsum" in table for each calendar file while running first
			        $cal_exclude_Array[$k+1] = "lore ipsum";  			// ensures that $cal_exclude_Array and $cal_files_Array has same number of items
				}								// if $CalendarExclude = "lore ipsum" nothing is removed from calendar information !!
			}
			$CalendarOptions['caldav2ics_calendar_excludes'] = serialize($cal_exclude_Array);
			update_option('caldav2ics_calendar_excludes', serialize($cal_exclude_Array));
		...
		if (!isset($_POST['updateSettingsjpg'])) {
			$this->bl_cron_exec();	// create ICalFile when 'submit' pressed ! JPG update only excute routine when intended
		}
		// HTML for the page
		...
		<?php if($exclude) { ?>
		<tr valign="top"><th scope="row">Exclude Phrase Definition </th>
		<?php }
		?> 
		<td>
			<?php
			$cal_exclude_Array = unserialize($CalendarOptions['caldav2ics_calendar_excludes']);
			if($exclude) {
				if(is_array($cal_exclude_Array)) {
				foreach($cal_exclude_Array as $key=>$val) { ?>
					<input type="text" name="caldav2ics_calendar_excludesKey[]" value="<?php _e($key, 'wp-caldav2ics') ?>" style="width:40px;" disabled="disabled" />
					<input type="text" name="caldav2ics_calendar_excludes[]" class="calendars_options" value="<?php _e($val, 'wp-caldav2ics') ?>" /><br />
				<?php } 
				} ?>
			<input type="text" id="addNewKey" name="caldav2ics_calendar_excludes[]" value="" style="width:40px;" value="<?php _e($key+1, 'wp-caldav2ics') ?>" disabled="disabled" />
			<input type="text" id="addNewExclude" name="caldav2ics_calendar_excludes[]" class="calendars_options" value="" />
			<?php } ?>
			<?php
			if(!$exclude) {
				if(is_array($cal_exclude_Array)) {
				foreach($cal_exclude_Array as $key=>$val) { ?>
					<input type="hidden" name="caldav2ics_calendar_excludesKey[]" value="<?php _e($key, 'wp-caldav2ics') ?>" style="width:40px;" disabled="disabled" />
					<input type="hidden" name="caldav2ics_calendar_excludes[]" class="calendars_options" value="<?php _e($val, 'wp-caldav2ics') ?>" /><br />
				<?php } 
				} ?>
			   <input type="hidden" id="addNewKey" name="caldav2ics_calendar_excludes[]" value="" style="width:40px;" value="<?php _e($key+1, 'wp-caldav2ics') ?>" disabled="disabled" />
			   <input type="hidden" id="addNewExclude" name="caldav2ics_calendar_excludes[]" class="calendars_options" value="" />
			<?php } ?>
		...
		echo ("</li><li>");
		_e('Enable defintion of exclude phrase : FALSE !', 'wp-caldav2ics'); 	// JPG Code - only for private version of plugin
		echo ("</li><li>");
		...
		value="<?php _e('Save Changes', 'wp-caldav2ics') ?>"/> <br><hr><br>
	    <label for= "submit" style= "color:red; background:lightblue;"> <strong> Updates only settings verifying changes WITHOUT executing the program ! </strong> </label> 
		<input type="submit" class="button-primary" name="updateSettingsjpg"
		value="<?php _e("UPDATE Plugin Settings only !", 'wp-caldav2ics') ?>"/>
		</p>
		</form>
		</div>
		<?php
